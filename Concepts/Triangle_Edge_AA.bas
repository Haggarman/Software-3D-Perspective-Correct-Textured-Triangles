Option _Explicit
_Title "Triangle Edge AA 5 - Move with arrows, scale with + or - , press ESC to exit"
' 2025 Haggarman
'
' Anti-Alias the edges of triangles using partial coverage.
...
Dim Shared Coverage(Size_Render_X) As Single
...
' logging
Const log_float_lastindex = 4 * 2 - 1
Dim Shared record_float(log_float_lastindex) As Single
Dim Shared record_float_nextindex As Integer
record_float(0) = 0
...
Dim myscale_x As Single, myscale_y As Single
Dim tri2_yoffset As Single
Dim pin As vec3d

main:
ExitCode = 0
FrameCounter = 0
myscale_x = 32 ' scale up a "unit right triangle" that is otherwise 0..1
myscale_y = 24

' render space pin location of the triangles
pin.x = 36.0
pin.y = 3.0
...
Do
    ...
    tri2_yoffset = myscale_x + 4.0 ' just a small gap below triangle 1

    ' logging
    record_float_nextindex = 0
    ...
    If FrameCounter < 60 Then
        ...
        vertexA.x = pin.x + 0 * myscale_x - 6
        vertexA.y = pin.y + 0 * myscale_y - 2
        vertexA.w = depth_w
        vertexA.u = 0 * T1_width * depth_w
        vertexA.v = 0 * T1_height * depth_w

        vertexB.x = pin.x + 1 * myscale_x
        vertexB.y = pin.y + 0 * myscale_y
        vertexB.w = depth_w
        vertexB.u = 1 * T1_width * depth_w
        vertexB.v = 0 * T1_height * depth_w

        vertexC.x = pin.x + 0 * myscale_x
        vertexC.y = pin.y + 1 * myscale_y
        vertexC.w = depth_w
        vertexC.u = 0 * T1_width * depth_w
        vertexC.v = 1 * T1_height * depth_w

        ThreeDots vertexA, vertexB, vertexC
        ...
        vertexA.x = pin.x + 0 * myscale_x
        vertexA.y = pin.y + 0 * myscale_y + tri2_yoffset
        vertexA.w = depth_w
        vertexA.u = 0 * T1_width * depth_w
        vertexA.v = 0 * T1_height * depth_w

        vertexB.x = pin.x + 1 * myscale_x
        vertexB.y = pin.y + 1 * myscale_y + tri2_yoffset
        vertexB.w = depth_w
        vertexB.u = 1 * T1_width * depth_w
        vertexB.v = 1 * T1_height * depth_w

        vertexC.x = pin.x + 0 * myscale_x - 6
        vertexC.y = pin.y + 1 * myscale_y + tri2_yoffset + 2
        vertexC.w = depth_w
        vertexC.u = 0 * T1_width * depth_w
        vertexC.v = 1 * T1_height * depth_w
    ...
    Else
        ...
        vertexC.x = pin.x + 1 * myscale_x + 6
        vertexC.y = pin.y + 1 * myscale_y + 2
        vertexC.w = depth_w
        vertexC.u = 1 * T1_width * depth_w
        vertexC.v = 1 * T1_height * depth_w

        ThreeDots vertexA, vertexB, vertexC
        If FrameCounter > 70 Then
            TexturedNonlitTriangle vertexA, vertexB, vertexC
        End If
        ...
        vertexB.x = pin.x + 1 * myscale_x + 6
        vertexB.y = pin.y + 0 * myscale_y + tri2_yoffset - 2
        vertexB.w = depth_w
        vertexB.u = 1 * T1_width * depth_w
        vertexB.v = 0 * T1_height * depth_w

        vertexC.x = pin.x + 1 * myscale_x
        vertexC.y = pin.y + 1 * myscale_y + tri2_yoffset
        vertexC.w = depth_w
        vertexC.u = 1 * T1_width * depth_w
        vertexC.v = 1 * T1_height * depth_w
    ...
    'Locate 1, 1
    'Dim i As Integer
    'For i = 0 To record_float_nextindex - 1
    '    Print record_float(i)
    'Next i
...
Sub ThreeDots (A As vertex5, B As vertex5, C As vertex5)
    _Dest WORK_IMAGE
    PSet (A.x, A.y), _RGB32(222, 105, 127)
    PSet (B.x, B.y), _RGB32(249, 249, 127)
    PSet (C.x, C.y), _RGB32(94, 249, 161)
End Sub
...
Sub TexturedNonlitTriangle (A As vertex5, B As vertex5, C As vertex5)
    ...
    draw_max_y = _Ceil(C.y) ' when using edge AA do not subtract 1
    ...
    ' Y Accumulators
    Static leg_x1 As Single, leg_x1_prev As Single
    Static leg_w1 As Single, leg_u1 As Single, leg_v1 As Single

    Static leg_x2 As Single, leg_x2_prev As Single
    ...
    '_Dest DISP_IMAGE
    'Locate 1, 1
    Static legx1_step_AB As Single
    ...
    While row <= draw_max_y
        ...
        If row = draw_middle_y Then
            ...
            legx1_step_AB = legx1_step ' copy previous
            ...
            'delta_x = Abs(leg_x2 - leg_x1) ' duplicated here
            'record_float(record_float_nextindex) = delta_x: record_float_nextindex = record_float_nextindex + 1

        End If

        Static subpixel_xstep As Single
        Static subpixel_xleft4 As Single
        Static subpixel_xleft3 As Single
        Static subpixel_xleft2 As Single
        Static subpixel_xleft1 As Single
        Static subpixel_xright4 As Single
        Static subpixel_xright3 As Single
        Static subpixel_xright2 As Single
        Static subpixel_xright1 As Single

        ' Horizontal Scanline
        delta_x = Abs(leg_x2 - leg_x1) ' TODO, the last pixel has delta_x = 0 at point C
        ...
        If 1 = 1 Then
            ...
            If leg_x1 < leg_x2 Then
                ...
                If delta_x >= (1 / 2048) Then
                    ' just use previous step if this check fails
                ...
                If leg_x1_prev < leg_x1 Then
                    ' Edge AA pixel starting point could be further left
                    col = _Ceil(leg_x1_prev)
                ...
                ' ending point is (2)
                If leg_x2 > leg_x2_prev Then
                    draw_max_x = _Ceil(leg_x2)
                    If draw_max_x > clip_max_x Then draw_max_x = clip_max_x
                Else
                    draw_max_x = _Ceil(leg_x2_prev)
                    If draw_max_x > clip_max_x Then draw_max_x = clip_max_x
                End If

                If row = draw_middle_y Then
                    subpixel_xstep = 0.25 * legx1_step_AB
                Else
                    subpixel_xstep = 0.25 * legx1_step
                End If

                subpixel_xleft4 = leg_x1
                subpixel_xleft3 = leg_x1 - subpixel_xstep
                subpixel_xleft2 = subpixel_xleft3 - subpixel_xstep
                subpixel_xleft1 = subpixel_xleft2 - subpixel_xstep

                subpixel_xstep = 0.25 * legx2_step
                subpixel_xright4 = leg_x2
                subpixel_xright3 = leg_x2 - subpixel_xstep
                subpixel_xright2 = subpixel_xright3 - subpixel_xstep
                subpixel_xright1 = subpixel_xright2 - subpixel_xstep
            ...
            Else
                ...
                If leg_x2_prev < leg_x2 Then
                    ' Edge AA pixel starting point could be further left
                    col = _Ceil(leg_x2_prev)
                    If col < clip_min_x Then col = clip_min_x
                Else
                    col = _Ceil(leg_x2)
                    If col < clip_min_x Then col = clip_min_x
                ...
                ' ending point is (1)
                If leg_x1 > leg_x1_prev Then
                    draw_max_x = _Ceil(leg_x1)
                    If draw_max_x > clip_max_x Then draw_max_x = clip_max_x
                Else
                    draw_max_x = _Ceil(leg_x1_prev)
                    If draw_max_x > clip_max_x Then draw_max_x = clip_max_x
                End If

                subpixel_xstep = 0.25 * legx2_step
                subpixel_xleft4 = leg_x2
                subpixel_xleft3 = leg_x2 - subpixel_xstep
                subpixel_xleft2 = subpixel_xleft3 - subpixel_xstep
                subpixel_xleft1 = subpixel_xleft2 - subpixel_xstep
                ...
                subpixel_xright4 = leg_x1
                subpixel_xright3 = leg_x1 - subpixel_xstep
                subpixel_xright2 = subpixel_xright3 - subpixel_xstep
                subpixel_xright1 = subpixel_xright2 - subpixel_xstep

            End If

            ' be dum. better to be redundant than never start writing.
            Static cvg As Single

            Static subpixel_col As Long
            subpixel_col = col
            While subpixel_col < draw_max_x
                Coverage(subpixel_col) = 0.0
                subpixel_col = subpixel_col + 1
            Wend
            ...
            While col < draw_max_x
                cvg = 0.0
                If (col >= subpixel_xleft4) And (col <= subpixel_xright4) Then cvg = cvg + 0.25
                If (col >= subpixel_xleft3) And (col <= subpixel_xright3) Then cvg = cvg + 0.25
                If (col >= subpixel_xleft2) And (col <= subpixel_xright2) Then cvg = cvg + 0.25
                If (col >= subpixel_xleft1) And (col <= subpixel_xright1) Then cvg = cvg + 0.25
                Coverage(col) = cvg
                'Print cvg;
                ...
                pixel_combine = pixel_value

                Static pixel_existing As _Unsigned Long

                If cvg < 0.998 Then
                    pixel_existing = _MemGet(screen_mem_info, screen_address, _Unsigned Long)
                    pixel_combine = _RGB32((  _red32(pixel_combine) - _Red32(pixel_existing))   * cvg + _red32(pixel_existing), _
                                           (_green32(pixel_combine) - _Green32(pixel_existing)) * cvg + _green32(pixel_existing), _
                                           ( _Blue32(pixel_combine) - _Blue32(pixel_existing))  * cvg + _blue32(pixel_existing))

                    ' x = (p1 - p0) * ratio + p0 is equivalent to
                    ' x = (1.0 - ratio) * p0 + ratio * p1
                End If

                _MemPut screen_mem_info, screen_address, pixel_combine
