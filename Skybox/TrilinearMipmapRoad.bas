...
' (M)ode 0 = no mipmap, Mode 1 = single mipmap per horizontal pixel, Mode 2 = trilinear per span.
'
...
Sub TwoTextureTriangle (A As vertex10, B As vertex10, C As vertex10)
    ...
    Static ds1_step As Single, dt1_step As Single
    ...
    Static ds2_step As Single, dt2_step As Single
    ...
    ds2_step = delta2.s / delta2.y
    dt2_step = delta2.t / delta2.y
    ...
    If delta1.y > (1 / 256) Then
        ...
        ds1_step = delta1.s / delta1.y
        dt1_step = delta1.t / delta1.y
    End If
    ...
    Static tex_s1 As Single, tex_t1 As Single
    ...
    Static tex_s2 As Single, tex_t2 As Single
    ...
    tex_s1 = A.s + prestep_y1 * ds1_step
    tex_t1 = A.t + prestep_y1 * dt1_step
    ...
    tex_s2 = A.s + prestep_y1 * ds2_step
    tex_t2 = A.t + prestep_y1 * dt2_step
    ...
    While row <= draw_max_y
        ...
        If row = draw_middle_y Then
            ...
            tex_s1 = B.s + prestep_y1 * ds1_step
            tex_t1 = B.t + prestep_y1 * dt1_step
        End If
        ...
        If delta_x >= (1 / 2048) Then
            ...
            If leg_x1 < leg_x2 Then
                ...
                tex_s_step = (tex_s2 - tex_s1) / delta_x
                tex_t_step = (tex_t2 - tex_t1) / delta_x
                ...
                tex_s = tex_s1 + prestep_x * tex_s_step
                tex_t = tex_t1 + prestep_x * tex_t_step
            ...
            Else
                ...
                tex_s_step = (tex_s1 - tex_s2) / delta_x
                tex_t_step = (tex_t1 - tex_t2) / delta_x
                ...
                tex_s = tex_s2 + prestep_x * tex_s_step
                tex_t = tex_t2 + prestep_x * tex_t_step
            ...
            Static LOD_1M_fraction As Single
            ...
            If (LOD_max > 0) And (LOD_mode = 2) Then
                ...
                If LOD_squared > 1.0 Then
                    ...
                    LOD_1M_fraction = 1.0 - LOD_fraction
                    ...
                    If LOD_max > 0 Then
                        ...
                        Select Case LOD_mode
                            ...
                            Case 1:
                                ...
                                ' Non-Trilinear Mip-mapping selects one texture at a time.
                                ' When we set these two LODs equal, T3 read will not occur.
                                LOD_3 = LOD
                        End Select
        ...
        tex_s1 = tex_s1 + ds1_step
        tex_t1 = tex_t1 + dt1_step
        ...
        tex_s2 = tex_s2 + ds2_step
        tex_t2 = tex_t2 + dt2_step
