Option _Explicit
_Title "MultiViewport 7 3dfx"
' 2026 Haggarman
' Testing around with the 3dfx shoelace formula to calculate dSdx, dTdx, and dWdx
'
...
Type render_viewport
    min_x As Long
    min_y As Long
    max_x As Long
    max_y As Long
    cx_ratio As Single ' range 0..1
    cy_ratio As Single
End Type
...
' Viewing area clipping defaults
...
' Camera usually follows player, but make player location a distinct point
Dim vPlayerPsn As vec3d
vPlayerPsn.x = 0.0
vPlayerPsn.y = 0.0
vPlayerPsn.z = 0.0

Dim vCameraPsn As vec3d ' location of camera in world space
vCameraPsn = vPlayerPsn
...
' WORK_IMAGE Scaling
Dim Shared halfWidth As Single
Dim Shared halfHeight As Single
halfWidth = Size_Render_X / 2
halfHeight = Size_Render_Y / 2

' Centerpoint of the viewport in WORK_IMAGE
' defaulted to exact center
Dim Shared View_CX As Single
Dim Shared View_CY As Single
View_CX = halfWidth
View_CY = halfHeight

' Projected WORK_IMAGE Coordinate List
...
' Screen clipping
Dim mainRenderViewport(2) As render_viewport
' These are just arbitrary, define how you want to arrange the viewpoints
' range is from (0,0) to (Size_Render_X, Size_Render_Y)
mainRenderViewport(0).min_x = 10
mainRenderViewport(0).max_x = Size_Render_X / 2 - 1
mainRenderViewport(0).cx_ratio = 0.5

mainRenderViewport(0).min_y = 2
mainRenderViewport(0).max_y = Size_Render_Y / 2 - 1
mainRenderViewport(0).cy_ratio = 0.5

mainRenderViewport(1).min_x = 2
mainRenderViewport(1).max_x = Size_Render_X / 2 - 1
mainRenderViewport(1).cx_ratio = 0.5

mainRenderViewport(1).min_y = Size_Render_Y / 2
mainRenderViewport(1).max_y = Size_Render_Y - 2
mainRenderViewport(1).cy_ratio = 0.5

mainRenderViewport(2).min_x = Size_Render_X / 2
mainRenderViewport(2).max_x = Size_Render_X - 10
mainRenderViewport(2).cx_ratio = 0.5

mainRenderViewport(2).min_y = 5
mainRenderViewport(2).max_y = Size_Render_Y - 5
mainRenderViewport(2).cy_ratio = 0.5
...
Do
    ...
    For viewnum = 0 To 2
        ...
        SetRenderViewport mainRenderViewport(viewnum)
        vCameraPsn = vPlayerPsn
        ...
        Select Case viewnum
            ...
            Case 1:
                ...
                Vector3_Add vCameraPsn, vLookDir, vCameraTarget
                ' go up by 100 units to be looking down
                vCameraPsn.y = vCameraPsn.y + 100.0
                Matrix4_PointAt vCameraPsn, vCameraTarget, vCameraTripod, matCamera()
        ...
        For A = 0 To Sky_Last_Element
            ...
            If triCount > 0 Then
                ...
                ' Scale into viewport, then offset
                SX0 = pointProj0.x * halfWidth + View_CX
                SY0 = pointProj0.y * halfHeight + View_CY

                SX1 = pointProj1.x * halfWidth + View_CX
                SY1 = pointProj1.y * halfHeight + View_CY

                SX2 = pointProj2.x * halfWidth + View_CX
                SY2 = pointProj2.y * halfHeight + View_CY
            ...
            If triCount = 2 Then
                ...
                SX3 = pointProj3.x * halfWidth + View_CX
                SY3 = pointProj3.y * halfHeight + View_CY
        ...
        For A = 0 To Particle_Last_Element
            If particles(A).anim_state > 0 Then
                ...
                If pointView0.z >= Frustum_Near Then
                    ...
                    ' particles remain the same size but still exist in 3D

                    ' top left
                    SX1 = (pointProj0.x - particles(A).relWidth) * halfWidth * Frustum_Aspect_Ratio + View_CX
                    SY1 = (pointProj0.y - particles(A).relHeight) * halfHeight + View_CY

                    ' bottom right
                    SX2 = (pointProj0.x + particles(A).relWidth) * halfWidth * Frustum_Aspect_Ratio + View_CX
                    SY2 = (pointProj0.y + particles(A).relHeight) * halfHeight + View_CY
    ...
    While frametimestamp_delta_ms > frametime_fullframethreshold_ms
        ...
        If _KeyDown(18432) Then
            ' Up arrow
            Vector3_Add vPlayerPsn, vMove_Player_Forward, vPlayerPsn
        End If

        If _KeyDown(20480) Then
            ' Down arrow
            Vector3_Delta vPlayerPsn, vMove_Player_Forward, vPlayerPsn
        End If
...
Sub SetRenderViewport (VP As render_viewport)
    ' requires Size_Render_X, Size_Render_Y
    If VP.min_x >= VP.max_x Then Exit Sub
    If VP.min_y >= VP.max_y Then Exit Sub

    clip_min_x = _Max(VP.min_x, 0)
    clip_min_y = _Max(VP.min_y, 0)
    clip_max_x = _Min(VP.max_x, Size_Render_X) ' not (-1) because rounding rule drops one pixel on right so this can safely be set to one higher
    clip_max_y = _Min(VP.max_y, Size_Render_Y - 1)

    halfWidth = (clip_max_x - clip_min_x) * 0.5
    halfHeight = (clip_max_y - clip_min_y) * 0.5

    View_CX = (clip_max_x - clip_min_x) * VP.cx_ratio + clip_min_x
    View_CY = (clip_max_y - clip_min_y) * VP.cy_ratio + clip_min_y
